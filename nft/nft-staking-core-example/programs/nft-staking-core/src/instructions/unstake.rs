    ...
    pub fn unstake(&mut self, bumps: &UnstakeBumps) -> Result<()> {
        ...
        for attribute in &mut collection_attributes.attribute_list {
            if attribute.key == "total_staked" {
                // Parsing the string to a u32 number
                let current_staked: u32 = attribute.value.parse().unwrap_or(0);
                // Decrementing by 1 safely (saturating_sub prevents it from going below 0)
