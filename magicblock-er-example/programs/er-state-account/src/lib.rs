...
declare_id!("GJ6JJSEFtfwc2GcuszkRbHaMqfVeCKVTtoQbicFJdTyr");
...
pub mod er_state_account {
    ...
    pub fn consume_randomness(ctx: Context<ConsumeRandomness>, random_value: u64) -> Result<()> {
        ctx.accounts.consume_randomness(random_value)?;
        Ok(())
    }
}
